...
## Command-Line Arguments (Usage)
 ...
 - `-p <PREFIX>` (`--prefix=<PREFIX>`): Sets the prefix for the names of the
   non-`static` functions, globals and structs that will be generated by zebu.
   This is useful to avoid symbol conflicts when linking multiple parsers
   generated by zebu.
   The default value is "zebu".
 - `-t <TEMPLATE>` (`--template=<TEMPLATE>`): Sets which parser-template zebu
   should use when generating it's output. The default value is "just-tables".
