...
## Command-Line Arguments (Usage)
 ...
 - `-p PREFIX` (`--prefix=PREFIX`): Sets the prefix the generated code should
   ...
   avoid symbol conflicts when linking multiple parsers generated by zebu.
   The default value is "zebu".
 - `-t TEMPLATE` (`--template=TEMPLATE`): Sets which parser-template zebu should
   use when generating it's output. The default value is "just-tables".
